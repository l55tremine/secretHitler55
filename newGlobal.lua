...
function refreshBelowLibButtons()
	...
	BLBparameters.position = {-3, 0,5.5}
	...
	BLBparameters.position = {0, 0,5.5}
	libBoard.createButton(BLBparameters)
	...
	BLBparameters.position = {6, 0,5.5}
	libBoard.createButton(BLBparameters)
end
...
function recreateVotes(object, color, alt_click)
	...
	if Player[color].admin and started == true then
		for i, value in pairs(getAllObjects()) do
			if (value.tag == "Card") then
				for i2, value2 in pairs(colours) do
					if (value.getDescription() == value2.."'s Nein Card" or value.getDescription() == value2.."'s Ja Card") then
		...
		for i, value in pairs(colours) do
			if (Player[value] ~= nil and Player[value].seated) then
				local newJa = jaCopy.clone({position = {1,4,i*3}})
				newJa.setDescription(value .. '\'s Ja Card')
				newJa.setScale({1.51, 1, 1.51})
				...
				newJa.setLuaScript(
						...
						'end\r\n')
				--newJa.deal(1,value)
				newJa.setPosition(getObjectFromGUID(HIDDEN_ZONE_GUIDS[value]).getPosition())
				
				local newNein = neinCopy.clone({position = {3,4,i*3}})
				newNein.setDescription(value .. '\'s Nein Card')
				newNein.setScale({1.51, 1, 1.51})
				newNein.interactable = true
				newNein.setLock(false)
				newNein.setLuaScript(
						'collision = false -- workaround for rewind error\r\n' ..
						'\r\n' ..
						'function onObjectSpawn(spawnObject)\r\n' ..
						...
						'	end\r\n' ..
						'end\r\n')
				newNein.setPosition(getObjectFromGUID(HIDDEN_ZONE_GUIDS[value]).getPosition())
			end
		end
	end
end
